...
1. We will also use the image files that we used in [Deep Learning for Images](./image-processing/python) and the CalTech101 dataset that we used in [Classical Machine Learning Fundamentals](./classical-machine-learning).  If you have not downloaded those files within the past 90 days, please do so now.
    ...
    ```
1. Load Your Trained MNIST Model.  At the end of [Deep Learning for Images](./deep_learning_for_images) we saved the trained MNIST model `model1` in `model1.h5`.  Here will load that model and we can pick up right where we left off.
    * If you were not able to save the model at the end of Deep Learning for Images, you can re-run the training of the MNIST model here before we start the rest of the tutorial.  For your convenience, below is the complete code that will load and preprocess the MNIST data and define and train the model.  You can cut and paste the code here into a code cell in this notebook and run it.  
        {% include accordion-wrapped title="Model code" controls="dlfi-model-code" class="info" removediv="true" content="    from keras.datasets import mnist  
    ...
    * Load the model:
        ...
        ```
1. We have now loaded the trained MNIST model from Deep Learning for Images.  Since this is a new notebook, however, we do not have the actual MNIST data loaded. We copy the code for loading and preprocessing the MNIST data from the Deep Learning for Images notebook.
    ```python
    (X_train, y_train), (X_test, y_test) = mnist.load_data()
    X_train = X_train.reshape(X_train.shape[0], 28, 28, 1)
    X_test = X_test.reshape(X_test.shape[0], 28, 28, 1)
...
### Sections
  - **Printing Characteristics of the CNN** how to print textual summaries of the CNN architecture
  - **Visualizing Activations** how to filter an example image through thhe MNIST network and visualize the activations
  - **Inputting New and Different Data to the Network** how to process new data to be compatible with the MNIST network and the effects of showing a non-digit image to the network
  - **The VGG Network** an exploration of the VGG16 network.


## Printing Characteristics of the CNN
...
<h4 class="highlighted__heading">Your turn</h4>
...
### A layer-wise summary of input and output shapes
...
{% include alert class="question" title="Your turn" content="Does this summary reconcile with the discussion in Tutorial 3 about the architecture of the MNIST model?  You might find it helpful to refer to the Tutorial 3 slides with the visualization of the MNIST network." %}

These shapes are consistent with the discussion in the Tutorial 2 slides.

### A layer-wise summary of filter shape and parameters
...
### Wrangling the example input image dimensions
...
### Modify `model` to output activations after the first conv layer
...
### Visualizing the 32 activations of the first conv layer
...
### Modify model to visualize activations after the second conv layer
...
### Modify model to visualize activations after the max pool layer
{% include alert class="question" title="Your turn" content="Modify the code above to visualize the output after the max pool layer.  Note--you probably want to define a new variable for this model to avoid overwriting the other models from above." %}
...
### Modify model to visualize activations of the fully connected layers
...
{% include alert class="question" title="Your turn" content="Explore the activations of the network for other input images.  For your convenience, code cells from above have been copied here for you to modify." %}
...
## Inputting New and Different Data to the Trained Network
...
{% include alert class="question" title="Your turn" content="
...
{% include alert class="question" title="Your turn" content="Repeat the above analysis, but keep the digit as dark on a light background." %}
...
### Loading the VGG Network Trained on ImageNet
...
### Classification Capabilities of the VGG16 Network
...
{% include alert class="question" title="Your turn" content="What class does the VGG16 network think your data belong to?  If you don't have data with you, you can peruse the internet for images of something that you want to try classifying with the network.  Just save the image to the same directory as this notebook and use the code above to classify it." %}

{% include alert class="question" title="Your turn" content="Using an image of your choice, use the methods we learned above to explore the workings of the VGG16 model." %}

### Transfer Learning on the VGG16 Architecture
...
{% include alert class="question" title="Your turn" content="Using an image of your choice from CalTech101, use the methods we learned above to explore the workings of the new transfer-learned VGG16 model." %}
