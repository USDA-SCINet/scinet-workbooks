...
{% include table content="| Database     | Content | Examples | Role |
|--------------|---------|----------|------|
...
The foundation of bioinformatics is built on sequence repositories. Three **primary nucleotide archives**: {% include nav-hidden text="GenBank" url="#genbank-acc" %}, {% include nav-hidden text="ENA" url="#ena-acc" %}, and {% include nav-hidden text="DDBJ" url="#ddbj-acc" %}, 
...
ensuring a globally consistent, comprehensive collection of sequence data. Beyond raw archival data, **curated secondary resources** such as {% include nav-hidden text="RefSeq" url="#refseq-acc" %} or {% include nav-hidden text="miRBase" url="#mirbase-acc" %} 
...
{% include accordion title="GenBank <span class='h-note'>(primary) nucleotide sequences; submissions from researchers worldwide</span>" class="outline" controls="genbank-acc" icon=false %}
<div id="genbank-acc" class="accordion_content" markdown='1' hidden> 
...
- check if a curated **{% include nav-hidden text="RefSeq" url="#refseq-acc" %} record** is available for your sequence of interest (more stable for downstream use)  
...
{% include accordion title="ENA <span class='h-note'>(primary) nucleotide sequences & raw sequencing reads; includes SRA data</span>" class="outline" controls="ena-acc" icon=false %}
<div id="ena-acc" class="accordion_content" markdown='1' hidden> 
...
- when working with raw reads, check if the data is already in **processed form** (e.g., via {% include nav-hidden text="RefSeq" url="#refseq-acc" %} or {% include nav-hidden text="SRA" url="#sra-acc" %}) before downloading terabytes unnecessarily  
...
Due to its large and overlapping content with {% include nav-hidden text="GenBank" url="#genbank-acc" %}/{% include nav-hidden text="RefSeq" url="#refseq-acc" %}, it's also uncommon to mirror ENA entirely. 
...
{% include accordion title="DDBJ <span class='h-note'>(primary) nucleotide sequences & raw sequencing reads; mirrors ENA/SRA scope</span>" class="outline" controls="ddbj-acc" icon=false %}
<div id="ddbj-acc" class="accordion_content" markdown='1' hidden> 
...
- prefer using **{% include nav-hidden text="ENA" url="#ena-acc" %} or {% include nav-hidden text="NCBI" url="#genbank-acc" %} mirrors** if bandwidth to Japan is slow  
- when possible, download processed/**curated versions** (e.g., {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}) instead of raw submissions  
...
{% include accordion title="RefSeq <span class='h-note'>(secondary) curated, non-redundant reference sequences (DNA, RNA, protein)</span>" class="outline" controls="refseq-acc" icon=false %}
<div id="refseq-acc" class="accordion_content" markdown='1' hidden> 
...
<b>RefSeq</b> differs from {% include nav-hidden text="GenBank" url="#genbank-acc" %}/{% include nav-hidden text="ENA" url="#ena-acc" %}/{% include nav-hidden text="DDBJ" url="#ddbj-acc" %} in being curated and non-redundant - perfect for reference-based analysis.
...
- **secondary database**: built by curating and annotating submissions from primary archives ({% include nav-hidden text="GenBank" url="#genbank-acc" %}/{% include nav-hidden text="ENA" url="#ena-acc" %}/{% include nav-hidden text="DDBJ" url="#ddbj-acc" %})  
...
- annotations are **expert-curated**, regularly updated, and integrated with NCBI resources (e.g., Gene, {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %})  
...
- for human/mouse, RefSeq is closely coordinated with {% include nav-hidden text="GENCODE" url="#gencode-acc" %}/{% include nav-hidden text="Ensembl" url="#ensembl-acc" %} - check consistency  
...
Like {% include nav-hidden text="GenBank" url="#genbank-acc" %}, RefSeq is a large, curated database of genomic, transcript, and protein sequences maintained by NCBI. 
...
{% include accordion title="miRBase <span class='h-note'>(secondary) reference microRNA sequences, annotations, and nomenclature</span>" class="outline" controls="mirbase-acc" icon=false %}
<div id="mirbase-acc" class="accordion_content" markdown='1' hidden> 
...
and **structured annotations** of genes, transcripts, and genomic features. Databases such as {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, the {% include nav-hidden text="UCSC Genome Browser" url="#ucsc-acc" %}, 
and {% include nav-hidden text="GENCODE" url="#gencode-acc" %} deliver curated gene models, coordinate systems, and visualization tools, forming the foundation for alignment, 
...
{% include accordion title="Ensembl <span class='h-note'>genome assemblies, gene models, variation</span>" class="outline" controls="ensembl-acc" icon=false %}
<div id="ensembl-acc" class="accordion_content" markdown='1' hidden> 
...
Ensembl IDs (`ENS...`) are **internal to Ensembl** and release-dependent, while {% include nav-hidden text="RefSeq" url="#refseq-acc" %} accessions (`NM_`, `NP`, `NC_`) are cross-database stable references. Tools often support both, but you should keep track of which system your pipeline expects.
...
- integrates with {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, {% include nav-hidden text="UniProt" url="#uniprot-acc" %}, {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %}, {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}, and other resources  
...
- {% include nav-hidden text="GENCODE" url="#gencode-acc" %} is the preferred annotation set for **human and mouse**, though Ensembl distributes it directly  
...
{% include accordion title="UCSC Genome Browser <span class='h-note'>genome assemblies, annotations, tracks</span>" class="outline" controls="ucsc-acc" icon=false %}
<div id="ucsc-acc" class="accordion_content" markdown='1' hidden> 
...
Choose UCSC for **browsing and visualization**, integration, and coordinate conversion, using [utilities](https://genome.ucsc.edu/util.html): [Table Browser](https://genome.ucsc.edu/cgi-bin/hgTables), [BLAT](https://genome.ucsc.edu/cgi-bin/hgBlat), [LiftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver), [trackhub](https://daler.github.io/trackhub/). It uses the same underlying genome assemblies as Ensembl, but may differ in annotation sets and track organization. {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} is best for curated annotations and cross-species comparative genomics.
...
- differences in annotation sets compared to {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} or {% include nav-hidden text="RefSeq" url="#refseq-acc" %} (reproducibility warning)  
...
{% include accordion title="GENCODE <span class='h-note'>comprehensive gene & transcript annotations</span>" class="outline" controls="gencode-acc" icon=false %}
<div id="gencode-acc" class="accordion_content" markdown='1' hidden> 
...
- **secondary annotation resource**, distributed through {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} and {% include nav-hidden text="UCSC" url="#ucsc-acc" %}  
...
- limited to **human and mouse** (other species rely on {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} or {% include nav-hidden text="RefSeq" url="#refseq-acc" %})  
...
- human and mouse GENCODE are synchronized with {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} releases and available via {% include nav-hidden text="UCSC Genome Browser" url="#ucsc-acc" %}  
- for reproducibility: match the GENCODE release with the **genome build** (e.g., `GRCh38`)  
- use [BioMart](http://useast.ensembl.org/biomart/martview/e03bf4380d6cb103e56668f87af7a074)/[Ensembl API](https://rest.ensembl.org/) for ID conversions between {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, GENCODE  
...
Core resources include {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %}, {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}, and the [1000 Genomes Project](https://www.internationalgenome.org), along with specialized databases 
such as {% include nav-hidden text="gnomAD" url="#gnomad-acc" %} (population allele frequencies), [COSMIC](https://cancer.sanger.ac.uk/cosmic/login) (cancer mutations), and [HGMD](https://www.hgmd.cf.ac.uk/ac/index.php) (heritable disease mutations). 
...
{% include accordion title="dbSNP <span class='h-note'>broad archive of SNPs and small variants, covering all organisms</span>" class="outline" controls="dbsnp-acc" icon=false %}
<div id="dbsnp-acc" class="accordion_content" markdown='1' hidden> 
...
- integrates with {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}, {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, [Gene](https://www.ncbi.nlm.nih.gov/gene/), and [Genome](https://www.ncbi.nlm.nih.gov/genome/)  
...
- enables **frequency-based filtering** and prioritization using population submissions (though less extensive than {% include nav-hidden text="gnomAD" url="#gnomad-acc" %})  
...
- less suitable for **clinical interpretation** compared to {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}  
...
{% include accordion title="ClinVar <span class='h-note'>human variants with clinical interpretations, linking genotype to disease relevance</span>" class="outline" controls="clinvar-acc" icon=false %}
<div id="clinvar-acc" class="accordion_content" markdown='1' hidden> 
...
- integrates with: {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %}, {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, [OMIM](https://omim.org/), [MedGen](https://www.ncbi.nlm.nih.gov/medgen/)  
...
particularly in disease-related and diagnostic workflows. `IGV` and {% include nav-hidden text="UCSC Genome Browser" url="#ucsc-acc" %} can be used to visualize ClinVar variants in genome.
</div>
...
{% include accordion title="gnomAD <span class='h-note'>population-scale human variant frequencies, filtering rare from common variants</span>" class="outline" controls="gnomad-acc" icon=false %}
<div id="gnomad-acc" class="accordion_content" markdown='1' hidden> 
...
- **frequency only** - no direct clinical significance annotations (unlike {% include nav-hidden text="ClinVar" url="#clinvar-acc" %})  
...
capturing expression profiles across tissues, conditions, diseases, and treatments. Key public resources include the {% include nav-hidden text="Sequence Read Archive (SRA)" url="#sra-acc" %} for primary sequencing reads, 
the {% include nav-hidden text="Gene Expression Omnibus (GEO)" url="#geo-acc" %} for curated gene expression datasets, and {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %} for microarray and RNA-seq data submissions with rich experimental metadata. 
...
{% include accordion title="SRA <span class='h-note'>(primary) repository of raw sequencing reads from transcriptomics and other studies</span>" class="outline" controls="sra-acc" icon=false %}
<div id="sra-acc" class="accordion_content" markdown='1' hidden> 
...
SRA is a **primary archive** - it stores raw sequencing reads. <br>Use {% include nav-hidden text="GEO" url="#geo-acc" %} or {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %} for curated functional genomics datasets.
...
- part of the [INSDC](https://www.insdc.org/) with mirrored repositories at {% include nav-hidden text="ENA" url="#ena-acc" %} (EMBL-EBI) and {% include nav-hidden text="DDBJ" url="#ddbj-acc" %} (Japan)  
- integrates with {% include nav-hidden text="GEO" url="#geo-acc" %} (via linked submissions) and *BioProject*/*BioSample*  
...
- annotations or condition labels are often minimal - cross-reference with {% include nav-hidden text="GEO" url="#geo-acc" %}/*BioSample*  
...
- combine with {% include nav-hidden text="GEO" url="#geo-acc" %} or {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %} for study design and sample annotation
...
{% include accordion title="GEO <span class='h-note'>curated archive of functional genomics data from microarrays and sequencing</span>" class="outline" controls="geo-acc" icon=false %}
<div id="geo-acc" class="accordion_content" markdown='1' hidden> 
...
GEO provides both **processed data** (e.g., expression matrices) and links to **raw data** hosted in {% include nav-hidden text="SRA" url="#sra-acc" %}. It's ideal for reanalysis and method benchmarking.
...
- often linked to raw [FASTQ](/bioinformatics/resources/file_formats#fastq)/[BAM](/bioinformatics/resources/file_formats#sam--bam--cram) files in {% include nav-hidden text="SRA" url="#sra-acc" %}  
...
- check if raw data is linked to {% include nav-hidden text="SRA" url="#sra-acc" %} under the `SRA` tab of a study  
...
{% include accordion title="ArrayExpress <span class='h-note'>functional genomics data, focusing on experimental metadata and reusability</span>" class="outline" controls="arrayexpress-acc" icon=false %}
<div id="arrayexpress-acc" class="accordion_content" markdown='1' hidden> 
...
- integrates with {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, [Expression Atlas](https://www.ebi.ac.uk/gxa/home), and [BioStudies](https://www.ebi.ac.uk/biostudies/)  
...
ArrayExpress is EMBL-EBI's functional genomics archive, similar to {% include nav-hidden text="GEO" url="#geo-acc" %}, and includes transcriptomics, epigenetics, and proteomics datasets from microarrays and RNA-seq. 
Since it's tightly integrated with BioStudies and {% include nav-hidden text="ENA" url="#ena-acc" %}, users typically download only the specific studies they need.
...
Key resources in this space include the {% include nav-hidden text="Gene Ontology (GO)" url="#go-acc" %} for standardized function classification, {% include nav-hidden text="KEGG" url="#kegg-acc" %} for curated maps of metabolic and signaling pathways, 
and {% include nav-hidden text="Reactome" url="#reactome-acc" %} for expert-reviewed molecular interaction networks. These databases power enrichment tools (e.g., GSEA, DAVID, clusterProfiler) 
...
{% include accordion title="Gene Ontology (GO) <span class='h-note'>structured ontology, standardized functional terms for genes</span>" class="outline" controls="go-acc" icon=false %}
<div id="go-acc" class="accordion_content" markdown='1' hidden> 
...
- requires gene ID conversion to GO-compatible formats (e.g., Entrez, {% include nav-hidden text="Ensembl" url="#ensembl-acc" %})  
...
- match GO annotations to your genome build/version (e.g., {% include nav-hidden text="Ensembl" url="#ensembl-acc" %} release)  
...
```
4. Use Ensembl [BioMart](http://useast.ensembl.org/biomart/martview/e03bf4380d6cb103e56668f87af7a074) or {% include nav-hidden text="UniProt" url="#uniprot-acc" %} mapping tools to convert to Entrez/UniProt for GO analysis.
</div>
</div>
</div>
...
{% include accordion title="KEGG <span class='h-note'>broader biochemical molecular pathways, metabolism, diseases, and drugss</span>" class="outline" controls="kegg-acc" icon=false %}
<div id="kegg-acc" class="accordion_content" markdown='1' hidden> 
...
- links to {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, {% include nav-hidden text="UniProt" url="#uniprot-acc" %}, [NCBI Gene](https://www.ncbi.nlm.nih.gov/gene/), and [PubChem](https://pubchem.ncbi.nlm.nih.gov)  
...
- data structure is less ontology-driven than {% include nav-hidden text="Reactome" url="#reactome-acc" %} or [BioCyc](https://biocyc.org)  
...
{% include accordion title="Reactome <span class='h-note'>curated pathway database for molecular signaling and reaction networks</span>" class="outline" controls="reactome-acc" icon=false %}
<div id="reactome-acc" class="accordion_content" markdown='1' hidden> 
...
- integrates with {% include nav-hidden text="UniProt" url="#uniprot-acc" %}, {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, Gene Ontology (GO), [ChEBI](https://www.ebi.ac.uk/chebi/), [PubMed](https://pubmed.ncbi.nlm.nih.gov)  
...
- some pathways (e.g., microbial metabolism) are less represented than in {% include nav-hidden text="KEGG" url="#kegg-acc" %}  
...
{% include nav-hidden text="UniProt" url="#uniprot-acc" %} offers a comprehensive, curated catalog of protein sequences, isoforms, domains, and functional annotations across species, 
integrating experimental and predicted data. The {% include nav-hidden text="Protein Data Bank (PDB)" url="#pdb-acc" %} archives experimentally determined 3D structures of proteins, DNA/RNA, and complexes, 
...
{% include accordion title="UniProt <span class='h-note'>(secondary) protein sequences and functional annotations; Swiss-Prot and TrEMBL</span>" class="outline" controls="uniprot-acc" icon=false %}
<div id="uniprot-acc" class="accordion_content" markdown='1' hidden> 
...
- **secondary database**: integrates sequences from primary sources ({% include nav-hidden text="GenBank" url="#genbank-acc" %}, {% include nav-hidden text="ENA" url="#ena-acc" %}, {% include nav-hidden text="DDBJ" url="#ddbj-acc" %}, {% include nav-hidden text="RefSeq" url="#refseq-acc" %})  
...
- integrates with **{% include nav-hidden text="PDB" url="#pdb-acc" %} (structures)**, **Gene Ontology**, and pathway databases (e.g., {% include nav-hidden text="Reactome" url="#reactome-acc" %})  
...
- use the [UniProt ID mapping service](https://www.uniprot.org/id-mapping) to convert between UniProt IDs, {% include nav-hidden text="RefSeq" url="#refseq-acc" %}, {% include nav-hidden text="Ensembl" url="#ensembl-acc" %}, and other identifiers  
...
{% include accordion title="PDB <span class='h-note'>(primary) 3D structural data for proteins, nucleic acids, and complexes</span>" class="outline" controls="pdb-acc" icon=false %}
<div id="pdb-acc" class="accordion_content" markdown='1' hidden> 
...
- integrates with {% include nav-hidden text="UniProt" url="#uniprot-acc" %}, [Pfam](http://pfam.xfam.org), [SCOP2](https://www.ebi.ac.uk/pdbe/scop/)/[CATH](https://www.cathdb.info), [DrugBank](https://go.drugbank.com), and variant databases  
...
- cross-reference with {% include nav-hidden text="UniProt" url="#uniprot-acc" %} to link sequences and functions  
...
**`ascp`** ([Aspera](https://scinet.usda.gov/guides/data/datatransfer#data-transfer-to-ncbi); much faster for raw reads; only from {% include nav-hidden text="ENA" url="#ena-acc" %}/{% include nav-hidden text="SRA" url="#sra-acc" %} databases)
```bash
# syntax
...
- `prefetch` + `fasterq-dump` for {% include nav-hidden text="SRA" url="#sra-acc" %}  
- `gsutil` or AWS CLI for {% include nav-hidden text="gnomAD" url="#gnomad-acc" %} cloud storage  
...
- **raw reads** in FASTQ format to {% include nav-hidden text="SRA" url="#sra-acc" %} / {% include nav-hidden text="ENA" url="#ena-acc" %} / {% include nav-hidden text="DDBJ" url="#ddbj-acc" %} (these synchronize globally)  
- **processed expression data** to {% include nav-hidden text="GEO" url="#geo-acc" %} / {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %}  
- **protein sequences & functional annotation** to {% include nav-hidden text="UniProt" url="#uniprot-acc" %}  
- **3D macromolecular structures (DNA/RNA/protein/complexes)** to {% include nav-hidden text="PDB" url="#pdb-acc" %}  
...
- Submitting processed data (counts, assemblies) into {% include nav-hidden text="SRA" url="#sra-acc" %} - these belong in {% include nav-hidden text="GEO" url="#geo-acc" %}/{% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %}.  
...
- Link raw and processed data (e.g., {% include nav-hidden text="SRA" url="#sra-acc" %} and {% include nav-hidden text="GEO" url="#geo-acc" %}) for clarity and reuse.
</div>
</div>
...
consider using {% include nav-hidden text="RefSeq" url="#refseq-acc" %} for stable, non-redundant gene and transcript models, or {% include nav-hidden text="GENCODE" url="#gencode-acc" %}/[Ensembl Plants](https://plants.ensembl.org/index.html) for comprehensive annotations.  
...
- {% include nav-hidden text="RefSeq" url="#refseq-acc" %} is curated and non-redundant, while {% include nav-hidden text="GENCODE" url="#gencode-acc" %} and [Ensembl Plants](https://plants.ensembl.org/index.html) provide broader transcript diversity.  
...
- {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %} coverage is limited for non-human species.  
...
- Assuming {% include nav-hidden text="dbSNP" url="#dbsnp-acc" %}, {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}, or {% include nav-hidden text="gnomAD" url="#gnomad-acc" %} provide allele frequencies for plants or animals - these are human-focused resources.  
- Depositing raw reads in {% include nav-hidden text="SRA" url="#sra-acc" %} without sharing the [VCF](/bioinformatics/resources/file_formats#vcf--bcf) makes results difficult to reuse.  
...
- Always pair **raw reads** in {% include nav-hidden text="SRA" url="#sra-acc" %}/{% include nav-hidden text="ENA" url="#ena-acc" %}/{% include nav-hidden text="DDBJ" url="#ddbj-acc" %} with **VCF variant calls** in a suitable repository.  
...
- Store raw reads in {% include nav-hidden text="SRA" url="#sra-acc" %}/{% include nav-hidden text="ENA" url="#ena-acc" %}/{% include nav-hidden text="DDBJ" url="#ddbj-acc" %}, and publish derived frequency tables as supplementary data or in community repositories. 
...
- Using {% include nav-hidden text="gnomAD" url="#gnomad-acc" %} for filtering in plants or wildlife studies - gnomAD is human-only.  
...
- Use {% include nav-hidden text="GEO" url="#geo-acc" %} and {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %} to find curated expression datasets under various conditions.  
...
- Confusing raw data archives ({% include nav-hidden text="SRA" url="#sra-acc" %}) with curated expression repositories ({% include nav-hidden text="GEO" url="#geo-acc" %}, {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %}).  
</div>
...
- Always record the dataset accession (e.g., *GSE ID* in {% include nav-hidden text="GEO" url="#geo-acc" %}, *E-MTAB ID* in {% include nav-hidden text="ArrayExpress" url="#arrayexpress-acc" %}) for reproducibility.  
</div>
</div>
...
consider {% include nav-hidden text="Gene Ontology (GO)" url="#go-acc" %} for functional categories, {% include nav-hidden text="KEGG" url="#kegg-acc" %}/{% include nav-hidden text="Reactome" url="#reactome-acc" %} for pathway enrichment.  
...
- Applying human-only background sets (e.g., from {% include nav-hidden text="gnomAD" url="#gnomad-acc" %} or {% include nav-hidden text="ClinVar" url="#clinvar-acc" %}) to plants or wildlife studies is inappropriate.  
</div>
...
consider using {% include nav-hidden text="UniProtKB" url="#uniprot-acc" %} for protein sequence and functional annotation retrieval.
...
- Search by *gene name*, *accession*, or *organism* in {% include nav-hidden text="UniProt" url="#uniprot-acc" %}.  
...
- Confusion with {% include nav-hidden text="GenBank" url="#genbank-acc" %} - it accepts protein sequences but lacks curated knowledge; UniProtKB (Swiss-Prot + TrEMBL) is the best resource for protein annotations.  
...
consider querying the {% include nav-hidden text="Protein Data Bank (PDB)" url="#pdb-acc" %} for experimentally solved structures.  
