...
2.	`options` : modify the behavior of the command and usually start with a dash or double dash (--) eg cp -r (copy recursively)
...
To display a message, we use `echo`. `echo` is a command that prints out whatever you type in quotes. 
...
o	Press Ctrl + A to move the cursor to the beginning of the command.
o	press Ctrl + E to move the cursor to the end of the command.
...
1.	What is the output of this command: `echo "Current user: $(whoami)"`?
  ...
  3. Syntax error
  4. Login time of the current user 
...
3.	What is the purpose of the up-arrow key in the shell?
  1. Deletes the last command 
  2. Displays help options for a command
  3. Shows environment variables
  4. Repeats the previous command 
...
This gives the absolute path of your current location. 
Example output: 
`/home/username`
...
Relative path begins from your current location:  `cd project/`
...
Now, we are ready to practice in our working directory safely!
...
`Control + O` to save, `Enter` to confirm 
`Ctrl + X` to exit
...
7. To make the script executable: `chmod u+x test_run.sh`

8. Let's look at the change in permissions. Do you notice the difference?
`ls -lh test_run.sh` 
